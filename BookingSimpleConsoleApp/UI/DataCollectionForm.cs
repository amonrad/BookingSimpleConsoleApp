namespace BookingSimpleConsoleApp
{
    public class DataCollectionForm : IDataCollectionForm
    {
        ...
        public IBookable GetClientInput(List<IBookable> existingClients)
        {
            string enteredName;
            ...
            do
            {
                ...
                {
                    // Check if a customer with the entered name already exists.
                    IBookable existingClient = existingClients.Find(c => c != null && c.Name.Equals(enteredName, StringComparison.OrdinalIgnoreCase));
                    ...
                    {
                        // If the customer does not exist, create a new customer and add to the list. Print welcome message and return the new customer.
                        IBookable newClient = Factory.CreateClient(enteredName);
...
int.TryParse(userInput, out duration) && IsDurationValid(duration))
        ...
        public IBookable getEmployeeInput(List<IBookable> employees)
        {
            string enteredName;
            ...
            while (true)
            {
                ...
                foreach (IBookable employee in employees)
                {
                ...
                IBookable selectedEmployee = getEmployeeByName(enteredName, employees);
        ...
        public IBookable getLocationInput(List<IBookable> locations)
        {
            ...
            while (true)
            {
                ...
                foreach (IBookable location in locations)
                {
                ...
                IBookable selectedLocation = getLocationByName(enteredLocation, locations);
        ...
        private IBookable getEmployeeByName(string name, List<IBookable> employees)
        {
            foreach (IBookable employee in employees)
            {
        ...
        private IBookable getLocationByName(string nameOfLocation, List<IBookable> locations)
        {
            foreach (IBookable location in locations)
            {
